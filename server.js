...
<html lang="pt-BR"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/><title>Player TI&CIA · Catálogo</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;background:#f5f7fa;padding:24px}.header{background:#fff;border-radius:12px;padding:24px;margin-bottom:24px;box-shadow:0 2px 8px rgba(0,0,0,0.08)}h1{font-size:28px;font-weight:700;color:#1a202c;margin-bottom:20px}.controls{display:flex;gap:12px;flex-wrap:wrap;margin-bottom:16px}input,select,button{padding:10px 16px;border:1px solid #e2e8f0;border-radius:8px;font-size:14px;font-family:inherit}input[type="text"]{flex:1;min-width:280px}input[type="checkbox"]{width:auto;padding:0;margin-right:6px}button{background:#fff;cursor:pointer;transition:all 0.2s;font-weight:500}button:hover{background:#f7fafc}button:disabled{opacity:0.5;cursor:not-allowed}button.primary{background:#3182ce;color:#fff;border:none}button.success{background:#38a169;color:#fff;border:none}button.danger{background:#e53e3e;color:#fff;border:none}button.warning{background:#ed8936;color:#fff;border:none}.batch-actions{display:flex;gap:8px;align-items:center;padding:12px 0;border-top:1px solid #e2e8f0;margin-top:8px}.table-wrap{background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08)}table{width:100%;border-collapse:collapse}thead{background:#edf2f7}th{padding:14px;text-align:left;font-weight:600;font-size:13px;color:#4a5568}td{padding:16px;border-bottom:1px solid #f7fafc;font-size:14px}tr:hover{background:#f7fafc}tr.selected{background:#bee3f8}.badge{display:inline-block;background:#edf2f7;padding:4px 10px;border-radius:12px;font-size:12px}.muted{color:#a0aec0;font-size:13px}.pagination{display:flex;gap:16px;align-items:center;justify-content:center;padding:24px}.meta{text-align:center;color:#718096;padding:16px}</style></head><body><div class="header"><h1>🎵 Catálogo</h1><div class="controls"><select id="store"><option value="">Todas lojas</option><option value="itaipu">Itaipu</option><option value="macae">Macaé</option><option value="rio">Rio</option></select><input id="q" type="text" placeholder="Buscar..."/><label><input type="checkbox" id="onlyLikes"/> Likes</label><label><input type="checkbox" id="onlyDislikes"/> Dislikes</label><button class="primary" id="apply">Filtrar</button><button class="success" id="scan">Scan</button></div><div class="batch-actions"><label><input type="checkbox" id="selectAll"/> Selecionar todos</label><span id="selectedCount" class="muted">0 selecionados</span><button class="warning" id="batchLike" disabled>❤️ Like em lote</button><button class="danger" id="batchDislike" disabled>👎 Dislike em lote</button></div></div><div class="meta" id="meta"></div><div class="table-wrap"><table><thead><tr><th style="width:40px">Sel</th><th>Artista</th><th>Música</th><th>Loja</th><th>Métricas</th><th>Ações</th></tr></thead><tbody id="tbody"></tbody></table></div><div class="pagination"><button id="prev">←</button><span id="pageInfo"></span><button id="next">→</button></div><script>let limit=50,offset=0,total=0,selectedIds=new Set();function buildQueryParams(){const p=new URLSearchParams();p.set('limit',limit);p.set('offset',offset);const store=document.getElementById('store').value;const q=document.getElementById('q').value;const onlyLikes=document.getElementById('onlyLikes').checked;const onlyDislikes=document.getElementById('onlyDislikes').checked;if(store)p.set('store',store);if(q)p.set('q',q);if(onlyLikes)p.set('onlyLikes','1');if(onlyDislikes)p.set('onlyDislikes','1');return p}function updateBatchButtons(){const count=selectedIds.size;document.getElementById('selectedCount').textContent=count+' selecionado'+(count!==1?'s':'');document.getElementById('batchLike').disabled=count===0;document.getElementById('batchDislike').disabled=count===0}function toggleSelection(id,checked){if(checked){selectedIds.add(id)}else{selectedIds.delete(id)}updateBatchButtons()}async function load(){const p=buildQueryParams();const r=await fetch('/api/admin/tracks.json?'+p,{credentials:'include'});if(!r.ok){alert('Erro ao carregar: '+r.status);return}const js=await r.json();total=js.total||0;document.getElementById('meta').textContent='Total: '+total;document.getElementById('pageInfo').textContent='Pág '+(Math.floor(offset/limit)+1)+' de '+(Math.ceil(total/limit)||1);const tb=document.getElementById('tbody');tb.innerHTML='';js.items.forEach(x=>{const tr=document.createElement('tr');const isSelected=selectedIds.has(x.id);if(isSelected)tr.classList.add('selected');tr.innerHTML='<td><input type="checkbox" class="row-select" data-id="'+x.id+'"'+(isSelected?' checked':'')+'/></td><td>'+x.artist+'</td><td>'+x.title+'<div class="muted">'+x.filename+'</div></td><td><span class="badge">'+x.store+'</span></td><td>❤️ '+x.likes+' 👎 '+x.dislikes+' ▶️ '+x.plays+'</td><td><button class="danger btn-del" data-id="'+x.id+'">Del</button></td>';tb.appendChild(tr)});document.querySelectorAll('.row-select').forEach(cb=>{cb.onchange=()=>{toggleSelection(cb.dataset.id,cb.checked);cb.closest('tr').classList.toggle('selected',cb.checked)}});document.querySelectorAll('.btn-del').forEach(b=>{b.onclick=async()=>{if(!confirm('Deletar esta música?'))return;const fd=new FormData();fd.set('id',b.dataset.id);const res=await fetch('/api/admin/track/delete',{method:'POST',body:fd,credentials:'include'});if(res.ok){alert('✅ Música deletada com sucesso!');selectedIds.delete(b.dataset.id);load()}else{alert('❌ Erro ao deletar')}}})}document.getElementById('selectAll').onchange=function(){const checked=this.checked;document.querySelectorAll('.row-select').forEach(cb=>{cb.checked=checked;toggleSelection(cb.dataset.id,checked);cb.closest('tr').classList.toggle('selected',checked)})};document.getElementById('batchLike').onclick=async()=>{if(!confirm('Marcar '+selectedIds.size+' música(s) como LIKE?'))return;let success=0;for(const id of selectedIds){try{await fetch('/api/like',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({trackId:id,clientId:'admin_batch',like:true,storeId:'default'}),credentials:'include'});success++}catch(e){console.error('Erro ao marcar like:',e)}}alert('✅ '+success+' música(s) marcadas como LIKE!');selectedIds.clear();load()};document.getElementById('batchDislike').onclick=async()=>{if(!confirm('Marcar '+selectedIds.size+' música(s) como DISLIKE?'))return;let success=0;for(const id of selectedIds){try{await fetch('/api/like',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({trackId:id,clientId:'admin_batch',like:false,storeId:'default'}),credentials:'include'});success++}catch(e){console.error('Erro ao marcar dislike:',e)}}alert('✅ '+success+' música(s) marcadas como DISLIKE!');selectedIds.clear();load()};document.getElementById('apply').onclick=()=>{offset=0;selectedIds.clear();load()};document.getElementById('prev').onclick=()=>{offset=Math.max(0,offset-limit);load()};document.getElementById('next').onclick=()=>{if(offset+limit<total){offset+=limit;load()}};document.getElementById('scan').onclick=async()=>{const res=await fetch('/api/admin/scan',{method:'POST',credentials:'include'});if(res.ok){alert('✅ Scan concluído!');load()}else{alert('❌ Erro no scan')}};updateBatchButtons();load()</script></body></html>`);
});
